...
app.get('/getMonth', (req, res) => {
    ...
    con.query(sql, (err, result) => {
        if (err) return res.json({ Error: "Get months error in sql" });
        return res.json({ Status: "Success", Result: result })
    })
})


app.post('/month/create', (req, res) => {
    const monmon = req.body.monmon;
    const mondes = req.body.mondes;

    con.query("INSERT INTO months (monmon, mondes) VALUES (?, ?)", [monmon, mondes],
        (err, result) => {
            if (result) {
                res.send(result);
            } else {
                res.send({ message: "ENTER CORRECT Month DETAILS!" })
            }
        }
    )
})

app.get('/month/get/:id', (req, res) => {
    const id = req.params.id;
...
app.put("/month/update/:id", (req, res) => {
    const userId = req.params.id;
    const q = "UPDATE months SET `monmon`= ?, `mondes`= ? WHERE id = ?";
...
app.delete('/month/delete/:id', (req, res) => {
    const id = req.params.id;
    const sql = "Delete FROM months WHERE id = ?";
    con.query(sql, [id], (err, result) => {
        if (err) return res.json({ Error: "delete months error in sql" });
        return res.json({ Status: "Success" })
    })
})
